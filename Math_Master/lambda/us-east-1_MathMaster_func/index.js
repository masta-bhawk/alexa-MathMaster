...
const MathQuizHandler = {
    canHandle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        console.log("Inside MathQuizHandler");
        console.log(JSON.stringify(request));
        return request.type === "IntentRequest" &&
            (request.intent.name === "MathQuizIntent" || request.intent.name === "AMAZON.StartOverIntent");
    },
    handle(handlerInput) {
...
function getMathQuestion(handlerInput) {
    console.log("I am in getMathQuestion");
    //GENERATING THE RANDOM QUESTION FROM DATA
    const random = getRandom(0, data.length - 1);
    const num1 = getRandom(0, 500);
    const num2 = getRandom(1,99);
    const plusminus = getRandom(0, 1);

    var ans =0;
    var speech_plusminus = "";
    if (plusminus==0) {
        ans = num1 + num2;
        speech_plusminus = "plus";
    }
    else {
        ans = num1 - num2;
        speech_plusminus = "minus";
    }
    ...
    attributes.num1 = num1;
    attributes.num2 = num2;
    attributes.plusminus = plusminus;
    attributes.plusminus_speech = speech_plusminus;
